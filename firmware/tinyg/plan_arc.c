				   ...
				   uint8_t motion_mode)                 // defined motion mode
{
	...
	// set true if radius arc
	bool radius_f = fp_NOT_ZERO(cm.gf.arc_radius);

	// radius value must > minimum radius
    if ((radius_f) && (fabs(cm.gn.arc_radius) < MIN_ARC_RADIUS))
	{
        return (STAT_ARC_RADIUS_OUT_OF_TOLERANCE);
    }

    // setup some flags, set true if X axis has been specified
	bool target_x = fp_NOT_ZERO(flags[AXIS_X]);
	bool target_y = fp_NOT_ZERO(flags[AXIS_Y]);
	bool target_z = fp_NOT_ZERO(flags[AXIS_Z]);

	// set true if offset I has been specified
    bool offset_i = fp_NOT_ZERO(cm.gf.arc_offset[0]);
    bool offset_j = fp_NOT_ZERO(cm.gf.arc_offset[1]);
    bool offset_k = fp_NOT_ZERO(cm.gf.arc_offset[2]);
	...
	if (cm.gm.select_plane == CANON_PLANE_YZ)
	{
	...
	// capture the fully resolved offsets to gm
	cm_set_work_offsets(&cm.gm);

	// copy GCode context to arc singleton - some will be overwritten to run segments
	memcpy(&arc.gm, &cm.gm, sizeof(GCodeState_t));

	// set initial arc position from gcode model
	copy_vector(arc.position, cm.gmx.position);

	// set arc radius or zero
	arc.radius = _to_millimeters(radius);

	// copy offsets with conversion to canonical form (mm)
	arc.offset[0] = _to_millimeters(i);
	arc.offset[1] = _to_millimeters(j);
	arc.offset[2] = _to_millimeters(k);

	// P must be a positive integer - force it if not
	arc.rotations = floor(fabs(cm.gn.parameter));
	...
	*/
	...
	// enable arc to be run from the callback
	arc.run_state = MOVE_RUN;
	cm_finalize_move();
	return (STAT_OK);
}
...
static stat_t _compute_arc()
{
	...
	//**********************************************************************************************
	/*
     *  Test arc specification for correctness according to: http://linuxcnc.org/docs/html/gcode/gcode.html#sec:G2-G3-Arc
	...
	*/
    ...
    if ((err > ARC_RADIUS_ERROR_MAX) || ((err < ARC_RADIUS_ERROR_MIN) && (err > arc.radius * ARC_RADIUS_TOLERANCE)))
	{
        return (STAT_ARC_SPECIFICATION_ERROR);
    }
	...
	// if full circle you can skip the stuff in the else clause
	...
	// NB: hypot is insensitive to +/- signs
	...
	arc.arc_segments = floor(min3(arc_segments_for_chordal_accuracy, arc_segments_for_minimum_distance, arc_segments_for_minimum_time));

	//...but is at least 1 arc_segment
	arc.arc_segments = max(arc.arc_segments, 1);
 ...
 *		j = (y + (x * h_x2_div_d)) / 2
 *
 *** omputing clockwise vs counter-clockwise motion ************************************************
 *
